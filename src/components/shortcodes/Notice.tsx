import { humanize } from '@/lib/text-converter';
import { cn } from '@/lib/utils';
import React from 'react';

const typeStyles: Record<string, string> = {
    note: 'text-primary border-current',
    tip: 'text-primary-light border-current',
    info: 'text-secondary/80 border-current',
    warning: 'text-secondary border-current',
};

function Notice({ type, children }: { type: string; children: React.ReactNode }) {
    return (
        <div className={cn('mb-6 rounded-lg border px-8 py-6', typeStyles[type])}>
            <div className="flex items-center">
                ...
                <p className="my-0 ml-1.5 font-secondary text-xl font-semibold text-text-dark">{humanize(type)}</p>
            </div>
