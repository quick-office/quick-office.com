...
export const getListPage = async <C extends CollectionKey>(
    ...
    const data = (await getEntry(collectionName, documentId)) as CollectionEntry<C> | null;

    if (!data) {
        throw new Error(`No page found for the collection: ${collectionName} with filename: ${documentId}`);
    }
